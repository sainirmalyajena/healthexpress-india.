...
async function getSurgeries(searchParams: SearchParams) {
    ...
    // Base filters (Category & Insurance)
    const where: any = {};
    ...
    // If search query 'q' exists, we must fetch all matching candidates and filter in-memory
    // because Prisma/Postgres doesn't easily support "array element contains substring"
    if (searchParams.q) {
        const q = searchParams.q.toLowerCase();

        // Fetch all candidates that match the base filters
        const allCandidates = await prisma.surgery.findMany({
            where,
            orderBy: { name: 'asc' }
        });

        // Filter in memory for name OR symptoms
    ...
    // Standard DB-based pagination if no search query
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto">
                                    <Link
                                        href={`/surgeries${params.q ? `?q=${params.q}` : ''}${params.insurance ? `${params.q ? '&' : '?'}insurance=${params.insurance}` : ''}`}
                                        className={`block px-3 py-2 text-sm rounded-lg transition-colors ${!params.category ? 'bg-teal-50 text-teal-700' : 'text-slate-600 hover:bg-slate-50'
                                            }`}
                                    >
                                        All Categories
                                    </Link>
                                    {categories.map((cat) => (
                                        <Link
                                            key={cat}
                                            href={`/surgeries?category=${cat}${params.q ? `&q=${params.q}` : ''}${params.insurance ? `&insurance=${params.insurance}` : ''}`}
                                            className={`block px-3 py-2 text-sm rounded-lg transition-colors ${params.category === cat ? 'bg-teal-50 text-teal-700' : 'text-slate-600 hover:bg-slate-50'
                                                }`}
                                        >
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {[
                                        ...
                                        <Link
                                            key={option.value}
                                            href={`/surgeries?${params.q ? `q=${params.q}&` : ''}${params.category ? `category=${params.category}&` : ''}${option.value ? `insurance=${option.value}` : ''}`}
